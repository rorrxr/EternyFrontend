...
- **🔍 핵심 기능 요약약**: 전적 검색, 통계 분석, 실시간 랭킹 시스템
- **📊 데이터 소스**: [이터널리턴 공식 API](https://open-api.bser.io/)

## 🎯 프로젝트 기간
**1차 MVP 기간: 2025.12 ~ 2025.07**
...
### 1️⃣ Git Clone
```bash
git clone https://github.com/rorrxr/eterny-frontend.git
```
...
**선택 이유**: 게임 전적 데이터의 실시간 조회 및 캐싱이 중요한 서비스 특성상 서버 상태 관리에 최적화된 라이브러리가 필요. 자동 리패패치, 백그라운드 업데이트, 스테일 타임 설정을 통해 최신 게임 데이터 제공
...
- **매치 히스토리**: React Window 기반 무한 스크롤 최적화
...
- **HTTP 클라이언트**: Zustand 기반 최적화된 API 통신
...
### **OAuth 인증 화면**
- **소셜 로그인 페이지**: Google, Kakao OAuth 버튼 배치
